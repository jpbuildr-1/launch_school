...
one tucked away in the functools module, but we won't be using it in this challenge.
...
like map, but instead of returning a new collection, it just returns a single value.
...
will be 2, the starting value given by the third argument to reduce. The callback will return 20.
...
reduce always uses the callback like this. The first invocation gets passed the first element
of the iterable and the starting value given by reduce's third argument. It then passes the 
callback the second element of the iterable and the return value of the first invocation. 
This continues until all elements have been processed by the callback, and
the final return value of reduce is the return value of the last callback invocation.
