...
def add_to_rolling_buffer1(buffer, max_buffer_size, new_element):
    buffer.append(new_element) # Method append
    if len(buffer) > max_buffer_size:
        buffer.pop(0)
    return buffer

def add_to_rolling_buffer2(buffer, max_buffer_size, new_element):
    buffer = buffer + [new_element] # List Concatenation
    if len(buffer) > max_buffer_size:
        buffer.pop(0)
    return buffer
...
"""
# The key difference is 1 is mutating the list and 2 is creating and assigning a new list